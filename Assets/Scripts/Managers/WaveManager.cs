...
public class WaveManager : MonoBehaviour {
	...
	IEnumerator StartNextWave() {
		...
		if ((waveNumber + ((difficulty - 1) * waves.Length)) == 5) {
			GameObject.Find ("MessageCanvas").GetComponent<Canvas> ().enabled = true;//enable the message
			Destroy (GameObject.Find ("Door_a"));
		}
	}
