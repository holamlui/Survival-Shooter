...
public class HellephantAttack : MonoBehaviour {
	...
	public int bulletsPerVolley = 10;
	...
	void SpecialAttack() {
		...
		if (!landed) {
			...
			transform.position = new Vector3(transform.position.x, Mathf.Lerp(floatHeight, 78, landingTime), transform.position.z);
		}
		// Start lifting.
		else {
			...
			transform.position = new Vector3(transform.position.x, Mathf.Lerp(78, floatHeight, landingTime), transform.position.z);
		}
		...
		if (transform.position.y == 78) {
			...
			if (!usedSpecial) {
				for (int i = 0; i < numberOfBullets; i++) {
					...
					Instantiate(zombear, transform.position + new Vector3(0, 0.5f, 0), rot);
				}
