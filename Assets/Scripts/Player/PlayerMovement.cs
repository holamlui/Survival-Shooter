...
public class PlayerMovement : MonoBehaviour {
	...
	void Animating(float h, float v) {
		...
		//walk front & back
		Vector3 vec = Camera.main.transform.forward * Input.GetAxis("Vertical") + Camera.main.transform.right * Input.GetAxis("Horizontal");
		Vector3 localVec = transform.InverseTransformDirection (vec);
		if (localVec.z > 0.1) {
			anim.SetFloat ("WalkingSpeed", 1);
		} else if (localVec.z < 0.1) {
			anim.SetFloat ("WalkingSpeed", -1);
		}
	}
}